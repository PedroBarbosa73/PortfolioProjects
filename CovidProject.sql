...
EXEC DataToInsertPPV3 
    @Date = SELECT [date] FROM CovidDeaths ,
    @Location = SELECT location FROM CovidDeaths,
	@totaldeaths= SELECT total_deaths FROM CovidDeaths  ,
	@totalcases =SELECT total_deaths FROM CovidDeaths,
	@PeopleVaccinated=SELECT people_vaccinated FROM CovidDeaths ,
	@PeopleFullyVaccinated=SELECT people_fullyvaccinated FROM CovidDeaths ,
	@PerPPLOneVac=SELECT(people_vaccinated/population)*100 CovidDeaths ,
	@PerPPLFullyVac= SELECT(people_fully_vaccinated/population)*100CovidDeaths
...
CREATE PROCEDURE InsertIntoTable3 
    @DataToInsert DataToInsertPPV READONLY
AS
BEGIN 
    ...
    INSERT INTO #PercentPopulationVaccinated (Date, Location, Population, PeopleVaccinated, PeopleFullyVaccinated, PerPPLOneVac, PerPPLFullyVac)
    SELECT 
        Date,
        Location,
        Population,
        People_Vaccinated,
        People_Fully_Vaccinated,
        (People_Vaccinated / NULLIF(Population, 0)) * 100 AS PerPPLOneVac,
        (People_Fully_Vaccinated / NULLIF(Population, 0)) * 100 AS PerPPLFullyVac
    FROM CovidDeaths;
    
    SELECT * FROM #PercentPopulationVaccinated; -- Returns the data inserted into the temp table
END;
